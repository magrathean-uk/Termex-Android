...
class RealSshFlowTest {
    ...
    private val subscriptionState = MutableStateFlow<SubscriptionState>(
        SubscriptionState.SUBSCRIBED(mockk(relaxed = true))
    )
    ...
    fun localForwardOpensSshBanner() {
        ...
        dismissOnboarding()
        seedKeyServer(
            displayName = "${fixture.liveKey.serverName} forward",
            host = fixture.liveKey.host,
            port = fixture.liveKey.port,
            username = fixture.liveKey.username,
            keyName = fixture.liveKey.keyName,
        ...
        openPortForwardingFromServerList("${fixture.liveKey.serverName} forward")
        addLocalForward(localPort = localPort, remoteHost = "127.0.0.1", remotePort = 4223)
        startForward()
        acceptAllHostKeysIfNeeded()
        waitForText("Active")
    ...
    fun remoteForwardExposesServiceOnHost() {
        ...
        val localProbe = Socket().use { socket ->
            ...
            String(socket.getInputStream().readNBytes(64), StandardCharsets.UTF_8)
        }
...
for _ in range(20):
    ...
    private fun openServer(displayName: String) {
        ...
        if (hasNodeWithTag(AutomationTags.serverRowTag(serverId))) {
            composeRule.onNodeWithTag(AutomationTags.serverRowTag(serverId), useUnmergedTree = true).performClick()
        } else {
            composeRule.onAllNodesWithText(displayName, useUnmergedTree = true)[0].performClick()
        }
        composeRule.waitUntil(20_000) {
            hasNodeWithTag(AutomationTags.HOST_KEY_ACCEPT) ||
    ...
    private fun readBannerThroughSocksOnce(socksPort: Int, targetHost: String, targetPort: Int): String {
        Socket("127.0.0.1", socksPort).use { socket ->
            ...
            val headerRead = input.readNBytes(response, 0, 4)
            if (headerRead != 4) {
                error("Missing SOCKS response header ($headerRead bytes)")
            }
            ...
            if (addressLength > 0) {
                input.readNBytes(ByteArray(addressLength), 0, addressLength)
            }
            input.readNBytes(ByteArray(2), 0, 2)
