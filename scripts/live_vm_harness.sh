...
android_resolve_managed_avd_name() {
  local avd_home="$1"
  local requested="${2:-}"
  local fallback="${3:-dev35_aosp_atd_arm64-v8a_Pixel_8}"
  ...
  if [[ -x "$emulator_bin" ]]; then
    detected="$(ANDROID_AVD_HOME="$avd_home" "$emulator_bin" -list-avds 2>/dev/null | sed '/^[[:space:]]*$/d' | head -n 1 || true)"
  fi
