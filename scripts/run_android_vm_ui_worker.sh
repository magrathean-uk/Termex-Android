...
AVD_NAME="${TERMEX_ANDROID_AVD_NAME:-dev35_aosp_atd_arm64-v8a_Pixel_8}"
  ...
  for _ in $(seq 1 50); do
    if [[ -s "$REMOTE_FORWARD_HELPER_PORT_FILE" ]]; then
      ...
      if [[ "$HOST_TRANSPORT" == "adb_reverse" ]]; then
        "$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" reverse \
...
capture_failure_artifacts() {
  ...
  if "$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" get-state >/dev/null 2>&1; then
    "$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" exec-out screencap -p >"$FAILURE_SCREENSHOT" 2>/dev/null || true
    "$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" logcat -d -v time 2>/dev/null | tail -n 600 >"$FAILURE_LOGCAT" || true
  fi
...
cleanup() {
  if [[ -n "${REMOTE_FORWARD_REVERSE_PORT:-}" ]]; then
    "$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" reverse --remove "tcp:$REMOTE_FORWARD_REVERSE_PORT" >/dev/null 2>&1 || true
  fi
...
boot_ok="0"
for _ in $(seq 1 180); do
  booted="$("$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" shell getprop sys.boot_completed 2>/dev/null | tr -d '\r')"
  if [[ "$booted" == "1" ]]; then
    boot_ok="1"
    break
  fi
...
"$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" shell settings put global window_animation_scale 0
"$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" shell settings put global transition_animation_scale 0
"$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" shell settings put global animator_duration_scale 0

"$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" install -r "$APP_APK" >/dev/null
"$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" install -r "$TEST_APK" >/dev/null

if [[ "$HOST_TRANSPORT" == "adb_reverse" ]]; then
  "$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" reverse "tcp:$TERMEX_VM_PORT" "tcp:$TERMEX_VM_PORT"
fi
...
for test_name in "${tests[@]}"; do
  if [[ "$PRESERVE_APP_DATA" == "0" ]]; then
    ...
    "$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" shell pm clear "$TEST_APP_ID" >/dev/null || true
  elif [[ "${TERMEX_VM_PRESERVE_APP_PROCESS:-0}" != "1" ]]; then
    "$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" shell am force-stop "$APP_ID" >/dev/null 2>&1 || true
    "$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" shell am force-stop "$TEST_APP_ID" >/dev/null 2>&1 || true
  fi
  ...
  if ! "$ANDROID_HOME/platform-tools/adb" -s "$SERIAL" shell am instrument -w -r \
    "${instrument_args[@]}" \
